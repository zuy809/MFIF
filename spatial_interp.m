...
if should_use_spatial_interp_mex(in, str, transform)
    out = spatial_interp_mex(double(in), double(warp), char(str), char(transform), double(nx), double(ny));
    return;
end
...
function use_mex = should_use_spatial_interp_mex(in, str, transform)
persistent mex_available
if isempty(mex_available)
    mex_available = (exist('spatial_interp_mex', 'file') == 3);
end

if ~(isnumeric(in) && ismatrix(in) && mex_available)
    use_mex = false;
    return;
end
...
supported_method = strcmp(str, 'linear') || strcmp(str, 'nearest');
supported_transform = strcmp(transform, 'translation') || strcmp(transform, 'euclidean') || ...
    strcmp(transform, 'affine') || strcmp(transform, 'homography');

use_mex = supported_method && supported_transform;
end
